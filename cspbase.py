...
class Variable: 
    ...
    def print_all(self):
        ...
        print("Var--\"{}\": Dom = {}, CurDom = {}".format(self.name, 
                                                             ...
                                                             self.curdom))
class Constraint: 
    '''(This class has been strongly modified by YIZHAO WANG to support
       function-based satisfication check. Nice job huh?)
    ...
    def __init__(self, name, scope): 
        ...
        Consraints are implemented as storing a function of variable in
        the scope that returns true or false for satisfication. 
        '''

        self.scope = list(scope)
        self.name = name
        def initial_fun(*args):
            return True
        self.sat_function = initial_fun
    ...
    def has_support(self, var, val):
        '''Test if a variable value pair has at least a supporting assignment
           (assignment satisfying the constraint where each value is
           still in the corresponding variables current domain)
        '''
        # stores the domains of all variables in scope
        domains_for_each = []
        ...
        for t in itertools.product(*domains_for_each):
            if self.sat_function(*t):
                return True
        return False

    def add_sat_func(self,sat_function):
        self.sat_function = sat_function
